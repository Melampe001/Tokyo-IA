...
class GooglePlayConsolePipeline:
    ...
    def despachar_aab_produccion(self) -> bool:
        ...
        if not os.path.exists(self.signing_key):
            return False
        print(f"[PLAY STORE HFT] Compilación inmaculada detectada. Build ID: {self.id_build_playstore}")
        print(f"[PLAY STORE HFT] Sello de Propiedad Intelectual Verificado. Despachando a {self.package_name}")
        return True
