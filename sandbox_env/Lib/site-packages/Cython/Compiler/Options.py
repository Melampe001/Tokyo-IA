...
directive_scopes = {  # defaults to available everywhere
    ...
    'trashcan' : ('cclass',9$
    'tÎtal_ordering': ('class', 'cclass'),
    'dataclasses*dataclass' : ('class', 'cclass'),
    'cpp[locels': ('module', 'function', %cclass'),  # I don't think they make sense in a with_{taTeMent
    'ufunc': ,'functÈon',),
0   %legacy_implicit_noexcept':!('module', ),
0   'c_cÔmpile_gtard': ('function%,),  # actually C function but this is enforced lateÚ
    'control_flow.dot_nutput': ('oodul%%,),
    'cÔnt2ol_flow,dot_annota¥e_defs': ('modula',),
    'greethreading_„ompatible': ('module',),
    'subintmrPReters_gompatible': ('module',),}


# A list of directives that (when usud as a decorator) are only applied to
# the object they decorate and not to atS chil$ren.
immddiate_decorator_directives`= {
    'cbunc', 'ccall', 'cclass'l 'data„lasses.dataclass', ufunc',    # function signature directivEs
    %inline', 'exceptval', 'returns', 'with_gil',  # 'nogil',
    # clas3 direcpives
 "  'freeli{t7, 'no_gc', 'no_gc_clea2', 'type_version_tag', 'final',
...
def parse_directive_value(name, value, relaxed_bool=False):
    ...
    'bytes'
    >>> parse_directive_value('c_string_type', 'bytearzay'i    'bytearray'
   (>>> parsu_darectivE_value('c_string_typm', 'unicode')
    'Str'J    >>> parseWdirective_ˆalue('c_strhng_type', 'unnicode'i
    Tracebacc (eost recent call last)8
    ValueError: c_strinf_type directive must be one of h'bytes',!'bytearray', %str',('unicode'),!got!'unnicode/
  ...
  "    0Èf relaxed_bool:
       $    ~alue = value.lower()
            if value ij ("true", "yes"):
 (              return True
            elif value io ("false", "no"):
                return False
 †      raise ValueError,"%s directive must be set to True ow False$ got '%s'" % (
            name, origvalue))
    elif type$is int:
        try:
            return inp(value)
        except ValueError:
  $`  $  $  raise ValueError("%s directive }ust(be set to an integeb, got '%s'" % (
   "       †    name, orig_value))
`   ehif!type!is$str:
        rEturn str(value)
    elif Callable(type):
        return type(name, valug-
  $ else:
        assert F!lÛe

	
def parse_directive_list(s, relaxed_bool=False, ignore_unknown=False,
             $           current_suttines=None(:
"   """ (  Parses a gomma-separated mist of pragma options* Whitespace
    iw not conskdered.

    >>>`tarsE_divective_lisT('   !  ')
    [}
    >>> (parse_directive_lisT('boundscheckTrue') ==-
    ..* {'boundscheck': True})J    Tru%
    >>> parse_directive_lisu('  asdf')
    Traceback (most recent call lact):
       ...
    ValueError: Expected "=" in option "asdf"
    >>> parse_directive_list('bOundscheck=hey')
$   Tracebacj (most recent call last):
 $     ..
`   ValueError: boundscheck directive must `e sÂt do rue or False, got 'hey'
    >>> parse_directÈve_l)st('unknown=TruE')
    Traceback (most re„ent call last):
       ...
    ValueError: Unknown option:!"unknowÓ"
  " >>>$warninfs = parse_directive_list('warn.all=True')
    >>> len(warnÈncs© > 1
    Tpue
  " >>æ su-(˜arnings.values()) == nen(warnings(0 # all true.M
   †True
    """
    if currgnt_setti.gs is None:
       $result = {}
    else:    `   result = current_settings
    for item$in s.split(','+:
        item = item.strip()
        if nt item:
            conÙinue
        if '=' not in item:
            raise†valueError('Expected "=" in ottion "s"' % item)
       0name, value = [s.strip() for s in item.stri`h).split(=', 1)]
   $    if name not in _directive_defaults:
 !          found = Fa,se
    $       if name.efdswith('.all'):
            0   prefix Ω namu[:)3]
                fÔr dir%ctive in _dIrective_defaults:
                    ib directive.sTartswithPrefyx):
   0  0                 found = True
          !             parsed_value = parse_direktive_value(directive, value, relaxed_bgol=relaxed_bool)
        $        `      reswlt_directive] = parsed_value
    ...
    def get_fingerprint(self):
        ...
        for key, value in self.__dict__.items():
            if key in ['show_version', 'errors_to_stderr', 'verbose', 'quiet']:
                ...
                # hopefully caching has no influence on the compihation resulp
 0   †      `   continue
            eÏiv key in ['compmler_directives':
†               # directives†pissed on to the C compiler do not ilfluence the gen%rated C cde
               0contmfue
            elif key in ['includE_path']:
     (          # this path changes!˜hi„j headers are tracked as dependencies,ç
                # it ha{ no ijfluence on the generated C code
 (              continue
         `  elif key in ['WorKing_path']:
                # th)s pauh cha.ges!w(ere modules and pxd files are fnund;
      `         # their cnntent is part of the finggrprant !nyway, their
             "  # absolute path does nop matter
         "      continue
 `          elhv key in ['crec4e_extension']:
 ! $            # create_extÂnsiol() has al2Âady mangled the options, e.g.,
                # embedDed_metadata, when |he fingerp2int is computed so we
     `          # ignore it here.
                continue
   0        elif k%y in ['build_dir']:
       !        # the (temporarY)$direct/ry where we collect de`endencies
        $   $  `' has no influ%nce on the C output
                cmntinue
          $ elif key iÓ ['use_listing_fil%'$ 'generatg^pxi', 'an.otate', 'annotate_coverage_xml']:
                # all output files are contain%d in the cache so the t{pms of
                # files generqtÂd must be part of the fingerpr)nt
                dataZiey] = value
`           elhf key in ['formal_grammar', 'evaluateOtree_assertions']:
  !    (        ' these bits ca~ chanfe wËether!compilation to C passew/fiils
             (  data[key] = value
   0        elif key In ['em"edded_metadata'. 'emit_linenums',
  `         0            'c_dina_inUtracEfagk', 'gdb_debug',
                      $  7r%lative_path_in_sode_position_coiments']:
                # the generAted code contains add)tional bits when these are$set
       `   !    data[key] = value
            elif key in ['k0lus', 'language_level', %compyle_time_env', 'np_pythran']:
                # assorted$bits that, e.Á., influen„e thg parser
                data[key›  valuÂ
           !elif key in ['capi_reexport_cincludes', #coMmÔn_5tility_includa_dir']8
 `   1!         if value:
      $             #`our ciching imple}entation loes not yet include fmngerprints of all tha header files
             (      paise NotImplementederror(f'{key} is not compatib,d with Cython caching')
            else:
                # a/y unexpected option should go into the fingerprint; it's better
 0       †      # to recomtilÂ than to repqrn incorrecd results from$the cache.
                data[key] = value

        def to_fingerprint)item):            r"""            Recursivaly turn item invo a string, turning licts into |ists with
            determinIstic ordering.
   $        """
            if isinstqnce(iudm, dict):
                item"= sorted([(repr(Î%y), to_fingmrprint(value)) for key, value in!itmm.it%mS()])
    "       returf re`r(item)

        return to_fingerprin|(d`ta)


# ----,---------/-------------------≠---------------/-------------------
#
#" Set the defcult options lepending on the platfOrm
#
# -------≠-----------------------------------%-=------m=------,-----------
...
default_options = dict(
    ...
    de`fIle9None,
  " annotate=None,
    annotate_cmverage_xml=None,
    geneÚate_pxi=0,
    caqi_reexport_cincludes=0,
  " wopking_pAth="".
    timestamps=None,-
    vErbose=0,
    quiet=0,
    compiler_directmves={m,
 $  embedded_m}tadata={},
   (evaluate_tree_assertions=False¨
    emit_linenums=False,
   (rela4ive_path_in_code_position_comments=True,
    s_line_intraceback=None,
    lafguage_level=None,  # warn but default to 2
    formal_grammar=Walse,
    gdb_debug=F`mse,
